...
function lycan_boss_summon_wolves:OnSpellStart()
	if IsServer() then
		...
		if self:GetCaster():FindModifierByName( "modifier_lycan_boss_shapeshift" ) ~= nil then
			nHoundSpawns = self:GetSpecialValueFor("num_hound_ss_spawn")
			nHoundBossSpawns = self:GetSpecialValueFor("num_hound_boss_ss_spawn")
			nWerewolves = self:GetSpecialValueFor("num_werewolf_ss_spawn")
		end
