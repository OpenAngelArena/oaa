...
if IsServer() then
  ...
  function modifier_skeleton_global_aura_effect:OnAttackLanded(event)
    ...
    elseif target:IsHero() or target:IsClone() or target:IsTempestDouble() then
      ...
      damage_table.damage = self.bonus_dmg_creeps or ability:GetSpecialValueFor("skeleton_bonus_damage_against_creeps")
