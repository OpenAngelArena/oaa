...
function monkey_king_wukongs_command_oaa:CreateMonkeyRing(unit_name, number, caster, center, radius)
  ...
  local top_monkey = CreateUnitByName(unit_name, top_point, true, caster, caster:GetOwner(), caster:GetTeam())
  top_monkey:SetOwner(caster)
  top_monkey:AddNewModifier(caster, self, "modifier_monkey_clone_oaa", {})

  local angle_degrees = 360/number
  for i=1,number-1 do
    local direction = RotatePosition(center, QAngle(0,i*angle_degrees,0), top_direction)
    local point = center + direction*radius
    local monkey = CreateUnitByName(unit_name, point, true, caster, caster:GetOwner(), caster:GetTeam())
    monkey:SetOwner(caster)
    monkey:AddNewModifier(caster, self, "modifier_monkey_clone_oaa", {})
  end
end

function monkey_king_wukongs_command_oaa:RemoveMonkeys()
  local caster = self:GetCaster()
  local unit_name = "npc_dota_monkey_clone_oaa"
  -- Find all monkeys belonging to the caster on the map and kill them
  local allied_units = FindUnitsInRadius(caster:GetTeam(), Vector(0, 0, 0), nil, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_BASIC, bit.bor(DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS, DOTA_UNIT_TARGET_FLAG_INVULNERABLE, DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD), FIND_ANY_ORDER, false)
  for _,unit in pairs(allied_units) do
    if unit:GetUnitName() == unit_name then
      unit:AddNoDraw()
      unit:AddNewModifier(unit, nil, "modifier_kill", { duration = 0.5 })
    end
  end
...
function modifier_monkey_clone_oaa:OnCreated()
  ...
  --parent:SetBaseAttackTime(ability:GetSpecialValueFor("bat"))    --caster:GetBaseAttackTime()
	--parent:SetHasInventory(false)
	parent:SetCanSellItems(false)
  parent:SetNeverMoveToClearSpace(true)
  parent:AttackNoEarlierThan(ability:GetSpecialValueFor("attack_interval"))  -- Setting attack interval
end
