...
function ChatCommand:OnPlayerChat(keys)
  ...
  if location ~= nil and (IsInToolsMode() or GameRules:IsCheatMode() or self:IsGameStateCommand(location))  then
    ...
    if context == nil then
      ...
      print("Process text command")
      func(context, keys)
      print("Finish Process text command")
    end
  end
end
