function pullStaff (keys)
    ...
    ability.forced_distance = ability:GetLevelSpecialValueFor("pull_length", ability_level)
    ability.forced_speed = ability:GetLevelSpecialValueFor("pull_speed", ability_level) * 1/30  -- * 1/30 gives a duration of ~0.4 second push time (which is what the gamepedia-site says it should be)
    ability.forced_traveled = 0

end

function pullHorizontal( keys )
