...
- The output of the tests is very long, so truncating it into an output logfile and then reading for that is advisable. Example: `npm run test > .test_output.log 2>&1; tail -n 200 .test_output.log`.
- The error messages from the KV tests relate to the data structure and not the purpose of the files, so for example an error might say that a key name differs from base dota and should inherit it. An error like that indicates that base dota changed the variables and we need to change ours to match their new format.
- Abilities with a baseclass other than data driven or lua are implemented upstream by Valve, and therefore we must exactly match the sets of SpecialValues they use

## Hero and Ability Basics
- Every hero must have a `npc_heroes_custom.txt` entry.
- Every hero gets a KV file in `game/scripts/npc/heroes/`.
- Every hero has a max level of 50.
- Every ability that scales, as in anything with a max level above 1, has two more levels of scaling in OAA. We call those last two levels "OAA levels".
- Not every value needs to scale, sometimes they level out and don't change in OAA levels (somewhat common for things like cooldown), however every key should have values for every level, including OAA levels.
